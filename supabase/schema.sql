...
-- Políticas para prestamos: Lectura pública, escritura reservada a administradores
CREATE POLICY "Lectura pública de prestamos" ON public.prestamos
    FOR SELECT TO public USING (true);

CREATE POLICY "Escritura de prestamos reservada a administradores" ON public.prestamos
    FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'ADMIN')) WITH CHECK (public.has_role(auth.uid(), 'ADMIN'));
